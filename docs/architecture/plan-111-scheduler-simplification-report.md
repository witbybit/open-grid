...
```powershell
corepack pnpm --filter @open-grid/core exec vitest run src/perf/instrumentedBudgets.test.ts src/renderer/runtimePerformance.test.ts src/renderer/serverRuntimePerformance.test.ts src/renderer/frameCoordinator.test.ts src/renderer/gridScheduler.test.ts
corepack pnpm --filter @open-grid/core exec vitest run src/diagnostics/GridInstrumentation.test.ts src/engine/architectureGuards.test.ts
