...
- `pnpm build` is intentionally sequenced: `@open-grid/core` must build before `@open-grid/react` because the React package compiles against the core `dist` entrypoints.
