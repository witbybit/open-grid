...
import { Activity, BarChart3, RefreshCw, TrendingUp, Zap } from 'lucide-react';
...
export default function RealtimeDashboard({
	...
	const [eventLogs, setEventLogs] = useState<Array<{ id: number; time: string; msg: string }>>([]);
	...
	const refreshStats = useCallback(() => {
		...
		for (const rowId of selectedIds) {
			const price = parseFloat(String(api.cells.getField(rowId, 'price')));
			if (!Number.isNaN(price)) numericValues.push(price);
		}
			...
			<div className='w-full xl:w-80 flex flex-col gap-4 shrink-0 overflow-y-auto max-h-full xl:max-h-none pr-1.5'>
				<div className='p-4 rounded-xl border border-slate-800 bg-slate-900/30 flex flex-col gap-3'>
					...
					<div className='grid grid-cols-2 gap-2'>
						{[
							...
							<div key={String(label)} className='bg-slate-950/60 border border-slate-900 rounded-lg p-2.5'>
				...
				<div className='p-4 rounded-xl border border-slate-800 bg-slate-900/30 flex flex-col gap-3'>
					<h3 className='text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5'>
						...
						<span className='ml-auto text-[9px] text-slate-500'>{stats.count > 0 ? 'selection' : 'all rows'}</span>
					</h3>
