...
export function GridStatusBar({ className, style, left, right }: GridStatusBarProps) {
		...
		left ?? (
			<div className='flex items-center gap-3 text-[11px] font-semibold text-slate-400'>
				<span className='inline-flex items-center gap-1.5'>
					...
					<span className='text-slate-200 tabular-nums'>{dataRowCount}</span>
				</span>
				<span className='inline-flex items-center gap-1.5'>
					<span className='text-slate-500'>selected</span>
					<span className='text-slate-200 tabular-nums'>{selectedRowIds.length}</span>
				</span>
			</div>
		);
	...
	return (
		<div
			className={className ?? 'flex items-center justify-between gap-3 border-t border-slate-800 bg-slate-950/90 px-3 py-2'}
			style={style}
		>
			{leftContent}
