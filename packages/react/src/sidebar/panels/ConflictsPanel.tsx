	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: bg, overflow: 'hidden' }}>
			...
			<div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 10 }}>
				{conflicts.length === 0 ? (
					<div style={{ ...mutedStyle, textAlign: 'center', marginTop: 24 }}>No active conflicts.<br />Use api.addConflict() to register one.</div>
				) : (
					<>
						...
						{conflicts.map((c) => (
							<div
								key={c.id}
								style={{ fontSize: 11, color: text, padding: '7px 9px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 5 }}
							>
								...
								<div style={{ display: 'flex', flexDirection: 'column', gap: 2, fontSize: 10, marginBottom: 6 }}>
									<div><span style={{ color: mutedText }}>Base: </span><span>{String(c.baseValue ?? '—')}</span></div>
									<div><span style={{ color: mutedText }}>Local: </span><span style={{ color: '#22c55e' }}>{String(c.localValue ?? '—')}</span></div>
									<div><span style={{ color: mutedText }}>Remote: </span><span style={{ color: '#f59e0b' }}>{String(c.remoteValue ?? '—')}</span></div>
								</div>
								<div style={{ display: 'flex', gap: 4 }}>
									<button onClick={() => handleFocus(c)} style={{ fontSize: 10, padding: '1px 6px', borderRadius: 3, border: `1px solid ${borderColor}`, background: 'none', color: mutedText, cursor: 'pointer' }}>Focus</button>
									<button onClick={() => handleResolve(c, { strategy: 'local' })} style={{ fontSize: 10, padding: '1px 6px', borderRadius: 3, border: '1px solid rgba(34,197,94,0.4)', background: 'rgba(34,197,94,0.08)', color: '#22c55e', cursor: 'pointer' }}>Keep Local</button>
									<button onClick={() => handleResolve(c, { strategy: 'remote' })} style={{ fontSize: 10, padding: '1px 6px', borderRadius: 3, border: '1px solid rgba(245,158,11,0.4)', background: 'rgba(245,158,11,0.08)', color: '#f59e0b', cursor: 'pointer' }}>Use Remote</button>
								</div>
							</div>
						))}
						<button
