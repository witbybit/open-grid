...
describe('Architecture guardrails', () => {
	...
	it('reactHostBridge.ts is the only React-side file that imports @eregister/open-grid-core/internal', () => {
		...
		for (const file of files) {
			const content = readFileSync(resolve(REACT_ROOT, 'src', file), 'utf-8');
			expect(content, `${file} must not import @eregister/open-grid-core/internal directly`).not.toContain('@eregister/open-grid-core/internal');
		}
	});
	...
	it('demo app imports no internal package entry points (Plan 106)', () => {
		for (const file of collectSourceFiles(resolve(DEMO_ROOT, 'src'))) {
			...
			expect(content, `${file} must not import @eregister/open-grid-react internals by subpath`).not.toMatch(/from ['"]@open-grid\/react\//);
			expect(content, `${file} must not import @eregister/open-grid-core internals by subpath`).not.toMatch(/from ['"]@open-grid\/core\//);
		}
	});
