...
import { createInfiniteBlockScopeId, createServerPageScopeId } from './asyncRowModelRequestIdentity.js';
...
describe('async row-model request identity helpers', () => {
	...
	it('creates a stable server-page scope id', () => {
		expect(createServerPageScopeId()).toBe('server-page:active-page');
		expect(createServerPageScopeId()).toBe(createServerPageScopeId());
	});
});
