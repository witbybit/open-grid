...
class QueuedScheduler implements GridReplayScheduler {
	...
	public runAll(): void {
		while (this.turns.length) this.runOne();
	}
}
...
const trace: GridReplayTrace = {
	...
	initial: {
		rowIdField: 'id',
		columns: [{ field: 'id' }, { field: 'name' }, { field: 'count' }, { field: 'formula' }],
		rows: [{ id: 'r:雪\0', name: 'Before', count: 1, formula: '=count*2' }],
	},
	commands: [
...
describe('GridTraceReplay', () => {
	...
	it('stops at first divergence and keeps declared observations non-executable', () => {
		const scheduler = new QueuedScheduler();
		const replay = createGridTraceReplay(
			encoded({
				...trace,
				checkpoints: [{ at: 0, facts: { cells: [{ rowId: 'r:雪\0', colField: 'name', value: 'Wrong' }] }, causalEvidence: ['commit #42'] }],
			}),
			{ scheduler }
		).replay!;
		replay.play();
		...
		const withUnknownObservation = validateGridReplayTrace(
			encoded({ ...trace, observations: [...trace.observations!, { at: 3, kind: 'server-network-request', evidence: { url: 'redacted' } }] })
		);
	...
	it('compares declared invalidations, formula results, faults, and fallbacks as semantic facts', () => {
		...
		baseline.destroy();
		const matching = createGridTraceReplay(
			encoded({
				...trace,
				checkpoints: [
					{
						at: 0,
						facts: {
							invalidationKinds: facts.invalidationKinds,
							formulaResults: facts.formulaResults,
							faults: facts.faults,
		...
		matching.destroy();
		const divergent = createGridTraceReplay(
			encoded({
				...trace,
				checkpoints: [
					{
						at: 0,
						facts: {
							invalidationKinds: ['not-a-real-invalidation'],
							formulaResults: facts.formulaResults,
							faults: facts.faults,
							fallbacks: facts.fallbacks,
						},
						causalEvidence: ['render evidence'],
					},
				],
			})
		).replay!;
		expect(divergent.step()).toBe('diverged');
		expect(divergent.divergence).toMatchObject({
			index: 0,
			expected: { invalidationKinds: ['not-a-real-invalidation'] },
			actual: { invalidationKinds: facts.invalidationKinds },
			causalEvidence: ['render evidence'],
		});
		divergent.destroy();
	});
	...
	it('never writes to, subscribes to, or emits events from a live grid', () => {
		...
		const rows = new ClientRowModelController(live.getClientRowModelRuntime(), {
			columns: [{ field: 'id' }, { field: 'name' }],
			rows: [{ id: 'r:雪\0', name: 'Live' }],
		});
		let notifications = 0;
		...
		for (let index = 0; index < 100; index++) {
			const candidate =
				index % 2 === 0
					? JSON.stringify({
							...trace,
							commands: Array.from({ length: next() % 5 }, () => ({
								kind: 'set-cell',
								rowId: 'r:雪\0',
								colField: 'count',
								value: next() % 9,
