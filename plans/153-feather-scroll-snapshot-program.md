...
The cell display snapshot has been upgraded from a flat `className + text` cache into a canonical payload with explicit base/state/decoration class segments, canonical tokenization, and an explicit `contentKind`. Full-bind paints, prewarm snapshot production, and loading-row binds now all write through that shared snapshot builder. The scroll binder now also prefers fresh logical snapshots over warm DOM memory for visible primitive reuse and buffered offscreen reuse, including buffered portal preservation when a fresh portal snapshot exists. Snapshot freshness now includes `insightVersion`, `styleVersion`, `loadingVersion`, and `selectionVersion`, and prewarmed snapshots now carry focused/selected/read-only/style-rule/tooltip state when those visuals matter for horizontal or vertical re-entry. The approach-band prewarm path is also now budgeted in priority order: display-value priming for formula/valueGetter cells runs first, snapshot-rich visual enrichment runs second, and plain primitive text/empty snapshots consume only the remaining budget. Formula/valueGetter cells no longer need to wait for a later prewarm sweep after their display value has already been prepared: when the value is primed in the first pass, the runtime now mints the authoritative snapshot immediately in that same pass, both for plain primitive cells and for richer primitive cells that also need integrity/style/read-only/tooltip state. Visible primitive scroll binds no longer consult cached display values live when no snapshot exists; they now follow `fresh snapshot -> warm coherent primitive fallback -> explicit dirty placeholder`, which removes another direct semantic read from the hot path and makes snapshot authority the normal route for newly visible primitive cells. When insight-heavy primitive cells enter the viewport without a fresh snapshot, the binder now materializes a canonical compatibility snapshot once and reuses that authoritative payload instead of live-merging decorations into the DOM on every bind, and those authoritative binds no longer get needlessly re-queued for post-scroll repair just because the cell has insight layers. The same snapshot-authority tightening now applies to frozen portal cells: if a fresh portal snapshot already covers integrity state, the scroll binder keeps the portal visible without forcing another post-scroll repair pass solely due to insight presence, and if a visible portal bind is still version-fresh but the snapshot cache missed, the binder now materializes a warm compatibility snapshot before deciding whether fidelity repair is needed. Offscreen same-identity warm cells are also now snapshotted before preservation, so buffered primitive text/fallback state and buffered frozen portal state are promoted into canonical compatibility snapshots instead of surviving only as slot-local DOM memory. Cell slots now also track the exact visual versions they last rendered, which makes warm compatibility promotion and visibility-boundary refresh more honest: the runtime can distinguish truly fresh warm visuals from merely same-row same-column reuse, and warm visible cells are no longer marked dirty after scroll just because insight or style churn exists elsewhere in the grid. That mounted-version contract is now centralized in shared `cellSlot` helpers rather than open-coded separately in the binder and lane logic, and the full mounted-freshness predicate now lives there too so row/global identity freshness and visual-version freshness cannot silently diverge between scroll binder and visibility-boundary logic. This still does not finish Phase B, but it materially strengthens snapshot authority and shrinks the number of active-scroll branches that treat hidden warm DOM state as truth.
