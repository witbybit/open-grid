...
- **Progress (2026-06-14, branch rendering-architecture-v2-wip-3)**:
    ...
    - Test status: **core 549/549, react 100/100**, core+react builds clean.
    - **NOT done — recommended follow-ups (each its own focused pass; deferred to protect the green state at session end):**
        - **Phase 4b — pin animation**: needs per-cell reparent FLIP (center↔lane) + sticky interaction + lane-width interpolation through `LayoutTransitionController`. Hot-path-adjacent (rebind), modest payoff. STOP risk: clone-and-swap if sticky fights the reparent mid-animation.
        ...
        - **Phase 5 — pagination + status bar**: status bar is the low-risk next demo of the registry (parent `container`, origin `statusBarTop`, reserve `statusBarHeight`) but requires wiring the scroll-viewport height to subtract `bottomChromeHeight` so the render window reserves it (host/ResizeObserver measurement). Pagination row-slicing is a data-layer change (row pipeline) — larger.
